...
══════════════════════════════════════════════════════════════ */
const THEMES = {
  dark:      { process: '#00ff88', resource: '#00d2ff', req: '#ff9f1c', alloc: '#00d2ff', cycle: '#ff3d6b', shadow: 'rgba(0,0,0,0.6)', bg: '#0a0c10' },
  light:     { process: '#059669', resource: '#2563eb', req: '#d97706', alloc: '#2563eb', cycle: '#dc2626', shadow: 'rgba(0,0,0,0.15)', bg: '#f7f8fc' },
  cyberpunk: { process: '#00ffcc', resource: '#ff00ff', req: '#ffee00', alloc: '#ff00ff', cycle: '#ff0055', shadow: 'rgba(255,0,255,0.2)', bg: '#0d0015' },
  matrix:    { process: '#00ff41', resource: '#00cc33', req: '#88ff00', alloc: '#00cc33', cycle: '#ff4141', shadow: 'rgba(0,255,65,0.15)', bg: '#000d00' },
  ocean:     { process: '#34d399', resource: '#38bdf8', req: '#fb923c', alloc: '#38bdf8', cycle: '#f87171', shadow: 'rgba(0,0,0,0.7)', bg: '#020b18' },
};
...
function switchTheme(t) { ThemeManager.apply(t); }
...
const GraphManager = {
  ...
  init() {
    ...
    const options = {
      physics: {
        ...
        forceAtlas2Based: { gravitationalConstant: -60, centralGravity: 0.01, springLength: 130, springConstant: 0.05, damping: 0.6 },
        stabilization: { iterations: 150, fit: true },
      },
      interaction: { hover: true, tooltipDelay: 180, zoomView: true, dragView: true, multiselect: false, selectConnectedEdges: false },
      edges: {
      ...
      nodes: {
        ...
        chosen: { node: (v, id, sel, hov) => { if (hov) { v.shadowSize = 20; v.borderWidth = 3; } } },
      },
    };
  ...
  handleClick(params) {
    ...
    switch (state.mode) {
      case 'process':  if (!nodeId) this.addProcessNode(params.pointer.canvas); break;
      case 'resource': if (!nodeId) this.addResourceNode(params.pointer.canvas); break;
      case 'connect':  this.handleConnectClick(nodeId); break;
    }
  },
  ...
  addProcessNode({ x, y }) {
    ...
    state.nodesDS.add({
      ...
      nodeType: 'process', title: `Process ${id}`,
    });
    updateStats(); showToast(`Process ${id} added`); clearResults();
  },
  ...
  addResourceNode({ x, y }) {
    ...
    state.nodesDS.add({
      ...
      nodeType: 'resource', title: `Resource ${id}`,
    });
    updateStats(); showToast(`Resource ${id} added`); clearResults();
  },

  handleConnectClick(nodeId) {
    if (!nodeId) {
      if (state.connectSource) { state.connectSource = null; UIController.updateEdgeHint('Click source node…'); }
      return;
    }
    if (!state.connectSource) {
      ...
      const src = state.connectSource, tgt = nodeId;
      state.connectSource = null;
      state.network.unselectAll();
      if (src === tgt) { showToast('Self-loops not allowed ✕', 'error'); UIController.updateEdgeHint('Click source node…'); return; }
      if (state.edgesDS.get().some(e => e.from === src && e.to === tgt)) {
        showToast('Edge already exists ✕', 'error'); UIController.updateEdgeHint('Click source node…'); return;
      }
      const srcNode = state.nodesDS.get(src);
      const edgeType = srcNode.nodeType === 'process' ? 'req' : 'alloc';
      const T = THEMES[currentTheme];
      const clr = edgeType === 'req' ? T.req : T.alloc;
      state.edgesDS.add({ from: src, to: tgt, label: edgeType, edgeType,
        color: { color: clr, highlight: clr, hover: clr }, width: 2 });
      updateStats(); showToast(`Edge ${src}→${tgt} (${edgeType})`); clearResults();
  ...
  highlightCycle(cycleNodes, cycleEdgeIds) {
    const T = THEMES[currentTheme];
    state.nodesDS.update(cycleNodes.map(id => ({
      id,
      color: { background: hexToRgba(T.cycle, 0.22), border: T.cycle,
    ...
    state.edgesDS.update(cycleEdgeIds.map(id => ({
      id, color: { color: T.cycle, highlight: T.cycle, hover: T.cycle }, width: 3, dashes: [8, 4],
    })));
  },

  resetHighlights() {
    const T = THEMES[currentTheme];
    state.nodesDS.update(state.nodesDS.get().map(n => {
      const c = n.nodeType === 'process' ? T.process : T.resource;
      return { id: n.id, color: this._makeNodeColors(c),
               font: { color: c, face: 'Orbitron', size: 13, bold: true },
               shadow: { enabled: true, size: 10, color: T.shadow } };
    }));
    state.edgesDS.update(state.edgesDS.get().map(e => {
      const clr = e.edgeType === 'req' ? T.req : T.alloc;
      return { id: e.id, color: { color: clr, highlight: clr, hover: clr }, width: 2, dashes: false };
    }));
  },

  reset() {
    state.nodesDS.clear(); state.edgesDS.clear();
    state.processCount = 0; state.resourceCount = 0; state.connectSource = null;
    updateStats(); clearResults(); setMode('none');
  },
};
...
const DeadlockEngine = {
  ...
  detect() {
    ...
    edges.forEach(e => { if (adj[e.from]) adj[e.from].push({ to: e.to, edgeId: e.id }); });
    ...
    const dfs = (start) => {
      ...
      recStack[start] = true; parent[start] = null;
      ...
      while (stack.length) {
        ...
        if (!foundNext) { visited[node] = true; recStack[node] = false; stack.pop(); }
      }
      return false;
    };
    ...
    for (const node of nodes) {
      if (!visited[node]) {
        recStack[node] = true; parent[node] = null;
        if (dfs(node)) return this.traceCycle(cycleStart, cycleEnd, parent, edges);
        recStack[node] = false; visited[node] = true;
      }
    }
  ...
  traceCycle(cycleStart, cycleEnd, parent, edges) {
    const cycleNodes = [cycleEnd], cycleEdges = [];
    const backEdge = parent['__back__'];
    if (backEdge) cycleEdges.push(backEdge.edgeId);

    let cur = cycleEnd, iter = 0;
    while (cur !== cycleStart && iter++ < 200) {
      const p = parent[cur]; if (!p) break;
      cycleEdges.push(p.edgeId); cur = p.from; cycleNodes.push(cur);
    }
...
const AnalysisPanel = {
  ...
  _renderDeadlock(result) {
    ...
    ordered.forEach((id, i) => {
      ...
      const chip = el('span', `cp-node cp-cycle ${n.nodeType === 'process' ? 'cp-process' : 'cp-resource'}`, id);
    ...
    cpv.appendChild(el('span', `cp-node cp-cycle ${firstNode.nodeType === 'process' ? 'cp-process' : 'cp-resource'}`, ordered[0]));

    // Why section
    show('whySection');
    document.getElementById('whyBody').innerHTML = this._buildWhyHTML(cycleNodes);

    // Coffman conditions
    show('coffmanSection');
    document.getElementById('coffmanBody').innerHTML = this._buildCoffmanHTML(cycleNodes);

    // Process breakdown
    show('processSection');
    document.getElementById('processBody').innerHTML = this._buildProcessTable(cycleNodes);

    // Resolution
    show('resolutionSection');
    document.getElementById('resolutionBody').innerHTML = this._buildResolutionHTML(cycleNodes);
  ...
  _renderSafe() {
    ...
    hide('cycleSection'); hide('whySection'); hide('coffmanSection');
  ...
  _buildWhyHTML(cycleNodes) {
    const edges = state.edgesDS.get();
    const processes = cycleNodes.filter(id => state.nodesDS.get(id)?.nodeType === 'process');
    const resources = cycleNodes.filter(id => state.nodesDS.get(id)?.nodeType === 'resource');

    // Find what each deadlocked process holds and waits for
    const holdings = {}, waiting = {};
    processes.forEach(pid => { holdings[pid] = []; waiting[pid] = []; });
    edges.forEach(e => {
      if (e.edgeType === 'req'   && processes.includes(e.from)) waiting[e.from].push(e.to);
      if (e.edgeType === 'alloc' && processes.includes(e.to))   holdings[e.to].push(e.from);
    });
  ...
  _buildCoffmanHTML(cycleNodes) {
    ...
    */
    ...
    const conditions = [
      ...
      {
        icon: '🚫', name: 'No Preemption',
        desc: 'Resources cannot be forcibly taken from a process; they must be released voluntarily.',
        met: 'YES',
        detail: 'Standard RAG model assumes non-preemptive resource allocation.',
      },
      {
        icon: '🔄', name: 'Circular Wait',
    ...
    return conditions.map(c => {
      const badgeCls = c.met === 'YES' ? 'cbadge-yes' : c.met === 'NO' ? 'cbadge-no' : 'cbadge-maybe';
  ...
  _buildProcessTable(cycleNodes) {
    ...
    if (!nodes.length) return `<p style="font-size:11px;color:var(--text-muted)">No processes in graph.</p>`;

    return nodes.map(n => {
      const holds   = edges.filter(e => e.edgeType === 'alloc' && e.to   === n.id).map(e => e.from);
      const waitsFor= edges.filter(e => e.edgeType === 'req'   && e.from === n.id).map(e => e.to);
  ...
  _buildSafeSection() {
    ...
    const withoutReqs = nodes.filter(n => !edges.some(e => e.from === n.id && e.edgeType === 'req'));
    const withReqs    = nodes.filter(n =>  edges.some(e => e.from === n.id && e.edgeType === 'req'));
    const ordered     = [...withoutReqs, ...withReqs];

    const wrap = document.getElementById('safeBody');
    if (!ordered.length) { wrap.innerHTML = '<p style="font-size:11px;color:var(--text-muted)">No processes to order.</p>'; return; }
...
function setMode(newMode) {
  ...
  const map = { process: 'btnProcess', resource: 'btnResource', connect: 'btnConnect' };
  if (map[newMode]) document.getElementById(map[newMode]).classList.add('active');

  const wrapper = document.querySelector('.canvas-wrapper');
  wrapper.className = 'canvas-wrapper' + (newMode !== 'none' ? ` mode-${newMode}` : '');

  const labels = {
    process:  '◉  PROCESS MODE — Click empty canvas to add',
    resource: '▣  RESOURCE MODE — Click empty canvas to add',
    connect:  '⟶  CONNECT MODE — Click source, then target node',
    none:     '← Select a mode to begin',
  };
  document.getElementById('modeText').textContent = labels[newMode] || labels.none;
  UIController.updateEdgeHint('Click source node…');
  UIController.setStatus(newMode !== 'none' ? 'active' : 'idle',
    newMode !== 'none' ? newMode[0].toUpperCase() + newMode.slice(1) + ' Mode' : 'Ready');
}

function checkDeadlock() {
  if (!state.nodesDS.length) { showToast('Graph is empty — add nodes first', 'error'); return; }
...
function updateStats() {
  ...
  document.getElementById('statProcesses').textContent = nodes.filter(n => n.nodeType === 'process').length;
  document.getElementById('statResources').textContent = nodes.filter(n => n.nodeType === 'resource').length;
...
function showToast(msg, type = 'info') {
  ...
  t.textContent = msg;
  t.style.borderColor = type === 'error' ? 'var(--red)' : type === 'warn' ? 'var(--orange)' : 'var(--border-hi)';
  t.style.color       = type === 'error' ? 'var(--red)' : type === 'warn' ? 'var(--orange)' : 'var(--accent)';
...
function loadDemoGraph() {
  setTimeout(() => {
    ...
    const addN = (id, shape, pos, type) => {
      ...
      state.nodesDS.add({
        id, label: id, x: pos.x, y: pos.y, shape,
        color: { background: hexToRgba(c,0.12), border: c,
                 highlight: {background:hexToRgba(c,0.25),border:c},
                 hover:     {background:hexToRgba(c,0.25),border:c} },
        font: { color: c, face: 'Orbitron', size: 13, bold: true },
        widthConstraint: { minimum: 54, maximum: 54 },
        heightConstraint: { minimum: 54, maximum: 54 },
        nodeType: type, title: `${type} ${id}`,
      });
    };

    state.processCount = 3; state.resourceCount = 3;
    ...
    const addE = (from, to, type) => {
      ...
      state.edgesDS.add({ from, to, label: type, edgeType: type,
        color: { color: clr, highlight: clr, hover: clr }, width: 2 });
    };

    // Deadlock: P1→R1→P2→R2→P3→R3→P1
    addE('P1','R1','req');   addE('R1','P2','alloc');
    addE('P2','R2','req');   addE('R2','P3','alloc');
    addE('P3','R3','req');   addE('R3','P1','alloc');
